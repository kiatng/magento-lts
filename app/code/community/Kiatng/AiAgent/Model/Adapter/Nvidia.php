 ...
 * @author     Kiatng
 */
class Kiatng_AiAgent_Model_Adapter_Nvidia extends Kiatng_AiAgent_Model_Adapter_Abstract
{
    /**
     * API endpoint for Nvidia
     */
    const API_ENDPOINT = 'https://api.nvidia.com/v1/chat/completions';
    
    /**
     * Send a message to Nvidia and get a response
     *
    ...
    public function sendMessage($message, $chatHistory = array(), $context = array())
    {
        ...
        if (empty($apiKey)) {
            Mage::throwException(Mage::helper('aiagent')->__('Nvidia API key is not configured'));
        }
        ...
        try {
            ...
            if (isset($response['choices'][0]['message']['content'])) {
                ...
                Mage::throwException(Mage::helper('aiagent')->__('Invalid response from Nvidia API'));
            }
        } catch (Exception $e) {
            Mage::logException($e);
